...
balls.append({
    ...
    "to_x" : 3, # x 축 이동 방향, -3 이면, 3 이면 오른쪽으로
    "to_y" : -6, # y 축 이동 방향
    "init_spd_y" : ball_speed_y[0]}) # y 최초 속도

# 사라질 무기,공 접보 저장 변수
weapon_to_remove = -1 #이변수에 들어온 값들은 더이상 의미가 없으므로 리스트에서 제거
ball_to_remove = -1
...
# 3. 게임 캐릭터 위치 정의
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        # 가로벽에 닿았을 때 공 이동 위치 변경 (공이 튕겨 나오는 효과)
        ...
        ball_val["pos_x"] += ball_val["to_x"] # 실제로 이 값을 공에 적용
...
# 캐릭터 rect 정보 업데이트
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3 :
                    ...
                    # 나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect() # 현재 공다음의 나눠진 공 크기 가져오기 위해 인덱스에 +1
                    ...
                    # 왼쪽으로 튕겨나가는 공
                    balls.append({
                        "pos_x" : ball_pos_x + (ball_width/2) - (small_ball_width / 2), # 고의 x좌표
            ...
            continue # 안쪽 for 문 조건이 맞지 않으면 continue. 바깥쪽 for문 계속 진행   
        break # 안쪽 for문에서  break를 만나서  여기로 진입. 2중 for문을 한번더 빠져 나감
    # for 바깥 조건:
    #     바깥 동작
    #     for 안쪽 조건:
    #         안쪽 동작
    ...
    # 충돌된 공 혹은 무기 없애기
    if ball_to_remove > -1: # idx 값은 0 부터 시작 하므로...
        del balls[ball_to_remove] # balls 리스트에서 제거됨
        ball_to_remove = -1 # 다시 -1로 설정
...
# 5. 화면에 그리기
    ...
    for idx, val in enumerate(balls): # 공 그려 주기
