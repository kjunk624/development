...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택 하세요", \
        ...
        initialdir=r"D:\Python\PyExcise") # D:\Python\PyExcise\pygame_project\images") 
...
def merge_image():
    ...
    try:
        ...
        image_sizes = [] # [(width1, hdight1), (width2, height2),...]
        if img_width > -1:
            # width 값 변경
        ...
        # 게산식
        # 100 * 60 이미지가 있음 ->width를 80으로 줄이면 height?
        ...
        # 스케치북 준비
        if img_space > 0 : # 이미지 크기에 간격옵션을 더한 사이즈 마련
            total_height += (img_space * (len(images) - 1)) 
        
        result_img = Image.new("RGB", (max_width, total_height), (255, 255, 255)) # 배경색 흰색 지정
        y_offset = 0 # y위치 정보
    
        for idx, img in enumerate(images):
            #  width가 원본유지가 아닐때는 이미지 크기 조정
            if img_width > -1:
            ...
            progress = (idx + 1) / len(images) * 100 # 실제 percentage 정보를 계산
            p_var.set(progress)
            progress_bar.update()
